...
module.exports = {
    ...
    getPendingAppointments: (req,res) => {
        ...
        order by date desc`).then(dbRes => res.status(200).send(dbRes[0]))
        .catch(err => console.log(err))
    ...
    getPastAppointments: (req, res) => {
        ...
        order by a.date desc;`).then(dbRes => res.status(200).send(dbRes[0])).catch(err => console.log(err))
    },
    ...
    completeAppointment: (res,req) =>{
        ...
        where appt_id = ${apptId};`).then(dbRes => {
            res.status(200).send(dbRes[0])
        }).catch(err => console.log(err))
    },
